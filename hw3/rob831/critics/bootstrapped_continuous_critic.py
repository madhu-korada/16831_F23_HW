...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ob_no, ac_na, next_ob_no, reward_n, terminal_n = \
            ptu.from_numpy(ob_no), ptu.from_numpy(ac_na), ptu.from_numpy(next_ob_no), \
            ptu.from_numpy(reward_n), ptu.from_numpy(terminal_n)
        
        for i in range(self.num_target_updates):
            V_s_prime = self.forward(next_ob_no)
            V_s_prime[terminal_n == 1] = 0
            # Q_s_a = reward_n + self.gamma * V_s_prime
            with torch.no_grad():
                Q_s_a = reward_n + self.gamma * V_s_prime
